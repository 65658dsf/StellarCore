...
import (
	...
	"github.com/65658dsf/StellarCore/client/visitor"
...
func (ctl *Control) handleReqWorkConn(_ msg.Message) {
	...
	if err = ctl.sessionCtx.AuthSetter.SetNewWorkConn(m); err != nil {
		xl.Warnf("工作连接认证错误: %v", err)
		workConn.Close()
		return
	}
	if err = msg.WriteMsg(workConn, m); err != nil {
		xl.Warnf("工作连接写入数据错误: %v", err)
		workConn.Close()
		return
	}
	...
	if err = msg.ReadMsgInto(workConn, &startMsg); err != nil {
		xl.Tracef("工作连接在接收StartWorkConn消息前关闭: %v", err)
		workConn.Close()
		return
	}
	if startMsg.Error != "" {
		xl.Errorf("StartWorkConn包含错误: %s", startMsg.Error)
		workConn.Close()
		return
	}
...
func (ctl *Control) handlePong(m msg.Message) {
	...
	if inMsg.Error != "" {
		xl.Errorf("Pong消息包含错误: %s", inMsg.Error)
		ctl.closeSession()
		return
	}
	ctl.lastPong.Store(time.Now())
	...
	if ctl.sessionCtx.Common.Transport.HeartbeatInterval > 0 {
		...
		sendHeartBeat := func() (bool, error) {
			...
			if err := ctl.sessionCtx.AuthSetter.SetPing(pingMsg); err != nil {
				xl.Warnf("心跳包认证错误: %v, 跳过发送心跳消息", err)
				return false, err
			}
