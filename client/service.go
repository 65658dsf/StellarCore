...
func (svr *Service) Run(ctx context.Context) error {
	...
	if svr.webServer != nil {
		go func() {
			log.Infof("管理面板监听地址: %s", svr.webServer.Address())
			if err := svr.webServer.Run(); err != nil {
				log.Warnf("管理面板退出，错误: %v", err)
			}
		}()
	}
	...
	if svr.ctl == nil {
		...
		return fmt.Errorf("登录服务器失败: %v。已启用登录失败退出选项，不再尝试重连", cancelCause.Err)
	}
...
func (svr *Service) login() (conn net.Conn, connector Connector, err error) {
	...
	xl.Infof("登录服务器成功，获取运行ID [%s]", loginRespMsg.RunID)
	return
}
